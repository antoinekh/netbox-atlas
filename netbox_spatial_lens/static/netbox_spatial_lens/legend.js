 ...
 * things it colours with `data-lens-filter`; anything on the page carrying
 * `data-lens-filterable="<that group>"` and a `data-band` joins in. Nothing here knows what a
 ...
 * by a class. Those listen for the `lens:filter` event instead, which carries the same decision.
 *
...
(function () {
  ...
  const legends = document.querySelectorAll('[data-lens-filter]');
  ...
  function buttonsOf(group) {
    return document.querySelectorAll(`[data-lens-filter="${group}"] [data-lens-band]`);
  }

  function describe(button) {
    return {
      label: (button.querySelector('.lens-legend__label') || button).textContent.trim(),
      count: Number((button.querySelector('.lens-legend__count') || {}).textContent) || 0,
    };
  }
  ...
  function apply(group) {
    ...
    buttonsOf(group).forEach(function (button) {
      const on = bands.has(button.dataset.lensBand);
    ...
    document.body.classList.toggle(`lens-filtering-${group}`, bands.size > 0);

    if (window.lensState) window.lensState.set(`band-${group}`, [...bands.keys()]);
  ...
  legends.forEach(function (legend) {
    const group = legend.dataset.lensFilter;
    picked.set(group, new Map());
    legend.addEventListener('click', function (event) {
      const button = event.target.closest('[data-lens-band]');
      if (!button || button.disabled) return;
      const bands = picked.get(group);
      const band = button.dataset.lensBand;
   ...
   * scripts run before DOMContentLoaded, so by then the page's own handlers for `lens:filter`
   * are in place and a restored band reaches them like a clicked one. */
  document.addEventListener('DOMContentLoaded', function () {
    if (!window.lensState) return;
    picked.forEach(function (bands, group) {
      const wanted = new Set(window.lensState.get(`band-${group}`));
      if (!wanted.size) return;
      buttonsOf(group).forEach(function (button) {
        if (!button.disabled && wanted.has(button.dataset.lensBand)) {
          bands.set(button.dataset.lensBand, describe(button));
        }
      });
      apply(group);
    });
