...
(function () {
  ...
  const config = document.getElementById('lens-rack-config');
  if (!config) return;

  const scopeLabel = document.querySelector('[data-lens-scope]');
  const allocBlocks = document.querySelectorAll('[data-lens-alloc]');
  const cablingBlocks = document.querySelectorAll('[data-lens-cabling]');
  const reservationPanels = document.querySelectorAll('[data-lens-reservation]');
  const traceCard = document.querySelector('[data-lens-trace-card]');
  const traceBody = document.querySelector('[data-lens-trace]');
  const traceSummary = document.querySelector('[data-lens-trace-summary]');
  ...
  function showCableCount(shown) {
    const badge = document.querySelector('[data-lens-cable-count]');
    if (!badge) return;
    const total = badge.dataset.lensCableCount;
  ...
  function showCabling(key) {
    const target = document.querySelector(`[data-lens-cabling="${key}"]`) ? key : 'rack';
    cablingBlocks.forEach(function (block) {
      block.hidden = block.dataset.lensCabling !== target;
    });
  }

  function showReservation(id) {
    reservationPanels.forEach(function (panel) {
      panel.hidden = panel.dataset.lensReservation !== String(id);
    });
  }
  ...
  function showAlloc(key, label) {
    ...
    // an empty panel.
    const target = document.querySelector(`[data-lens-alloc="${key}"]`) ? key : 'rack';
    allocBlocks.forEach(function (block) {
      block.hidden = block.dataset.lensAlloc !== target;
    });
  ...
  function markRows(selection, cableIds) {
    document.querySelectorAll('.lens-cable').forEach(function (row) {
  ...
  function renderTrace(data) {
    ...
    const rows = data.hops.map(function (hop) {
      ...
      return (
        `<li class="lens-hop lens-hop--${escape(hop.kind)}">` +
        `<span class="lens-hop__index">${hop.index}</span>` +
        `<span class="lens-hop__from">${from}</span>` +
        '<i class="mdi mdi-arrow-right text-muted"></i>' +
        `<span class="lens-hop__to">${to} ${rack}</span>` +
        '</li>'
      );
    });
    ...
    traceBody.innerHTML =
      ...
      `${escape(data.origin.label)}</p><ol class="lens-hops">${rows.join('')}</ol>` +
      (data.complete
        ? ''
  ...
  async function showTrace(termination) {
    ...
    try {
      ...
      console.error('netbox-spatial-lens could not trace:', error);
    }
  }
  ...
  document.addEventListener('click', function (event) {
    const row = event.target.closest('.lens-cable');
    if (row) pick(row);
  });
   ...
   *
   * `.lens-legend` is here because picking a band is an act on the drawing, not a click on
   * empty space beside it: left out, choosing a band cleared whatever was selected. */
  function ownsClick(target) {
    return Boolean(target.closest('.lens-cables, .lens-alloc, .lens-legend, [data-lens-reservation]'));
  }
  ...
  const find = document.querySelector('[data-lens-find]');
  // The box, not the input inside it: the stylesheet draws the warning on the bordered label.
  const findBox = find ? find.closest('.lens-find') : null;
  // A term carried in the URL, from a reload or a shared link.
  let term = ((window.lensState && window.lensState.get('find')[0]) || '').trim().toLowerCase();
  if (find) find.value = term;

  function setTerm(value) {
    term = value;
    if (window.lensState) window.lensState.set('find', term ? [term] : []);
