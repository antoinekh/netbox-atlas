...
from netbox_spatial_lens.geometry import autoplace_racks, rack_footprint_cm
from netbox_spatial_lens.models import CM_PER_UNIT, Floor, RackPlacement
    ...
    def _place(self, site, location, racks, options) -> int:
        ...
        floor, created = Floor.objects.get_or_create(
            ...
            defaults={
                ...
                'description': 'Created by lens_autoplace. Drag the racks to match the room.',
            },
        )
        self.stdout.write(f'{room}: floor "{floor.name}" ({"created" if created else "existing"})')

        if not options['replace']:
            racks = racks.filter(lens_placement__isnull=True)
        if not racks.exists():
