...
from netbox_atlas.palette import NO_DATA, categorical_colour
from netbox_atlas.site_overlays import distinct_colours, get_site_overlay, get_site_overlays
...
class ViewTestCase(AtlasTestCase):
    def setUp(self):
...
class FloorViewTest(ViewTestCase):
    ...
    def test_a_floor_with_no_data_says_so(self):
        ...
        self.assertContains(response, 'atlas-legend__count')

    def test_a_rack_links_inward_to_its_atlas_tab(self):
        response = self.client.get(self.floor.get_absolute_url())
        self.assertContains(response, f'/dcim/racks/{self.rack.pk}/atlas/')

    def test_the_editor_renders(self):
        response = self.client.get(reverse('plugins:netbox_atlas:floor_layout', args=[self.floor.pk]))
        self.assertEqual(response.status_code, 200)
...
class RackViewTest(ViewTestCase):
    ...
    def test_a_rack_with_devices_offers_a_way_to_find_one(self):
        ...
        response = self.client.get(reverse('dcim:rack_atlas', args=[rack.pk]))
        self.assertContains(response, 'data-atlas-find')
...
class RackCablingFinderTest(ViewTestCase):
    ...
    def setUp(self):
        ...
        from netbox_atlas.tests.base import cable

        cable(self.server.interfaces.first(), self.switch.interfaces.first())

    def _get(self):
        return self.client.get(reverse('dcim:rack_atlas', args=[self.rack.pk]))

    def test_the_cabling_list_is_a_finder(self):
        response = self._get()
        self.assertContains(response, 'data-atlas-finder="cables"')
        self.assertContains(response, 'data-atlas-finder-search')

    def test_every_cable_is_a_row_that_can_be_ticked(self):
        response = self._get()
        cable_id = self.rack.devices.first().interfaces.first().cable_id
        self.assertContains(response, f'data-atlas-pick="{cable_id}"')

    def test_a_row_names_both_ends_so_it_can_be_searched_for(self):
        response = self._get()
        self.assertContains(response, 'data-atlas-name')
    ...
    def test_the_card_says_where_the_cables_are(self):
        ...
        response = self._get()
        self.assertContains(response, 'data-atlas-cabling="rack"')
    ...
    def test_a_device_still_carries_its_own_cabling(self):
        ...
        response = self._get()
        self.assertContains(response, f'data-atlas-cabling="{self.switch.pk}"')

    def test_a_rack_with_no_cabling_offers_no_finder(self):
        bare = make_rack(self.site, name='R2', u_height=10)
        response = self.client.get(reverse('dcim:rack_atlas', args=[bare.pk]))
        self.assertNotContains(response, 'data-atlas-finder="cables"')


class SiteAtlasViewTest(ViewTestCase):
    """
    ...
    def _url(self):
        return reverse('dcim:site_atlas', args=[self.site.pk])
    ...
    def test_a_room_the_reader_may_not_see_is_not_listed(self):
        ...
        self.user.save()
        grant(self.user, 'netbox_atlas.floor', constraints={'name': 'Hall'})
...
class SiteLinkTest(ViewTestCase):
    ...
    def test_several_rooms_go_to_the_site_rather_than_to_one_of_them(self):
        ...
        self.assertEqual(self._url_for_site(), reverse('dcim:site_atlas', args=[self.site.pk]))

    def test_no_room_at_all_goes_to_the_site_atlas_tab(self):
        # The Atlas tab lists the site's racks and offers to add a floor; NetBox's own site page
...
class WorldViewTest(ViewTestCase):
    ...
    def test_the_finders_carry_every_site_and_a_way_to_filter_them(self):
        ...
        self.assertContains(response, 'data-atlas-finder="circuits"')
        self.assertContains(response, 'data-atlas-finder-search')
        self.assertContains(response, f'data-atlas-name="{self.site.name}"')

    def test_a_colouring_named_in_the_url_is_the_one_drawn(self):
        self.site.latitude, self.site.longitude = 51.5, -0.12
        self.site.status = 'planned'
        self.site.save()
        response = self.client.get(f'{reverse("plugins:netbox_atlas:world")}?overlay=status')
    ...
    def test_an_unknown_colouring_falls_back_rather_than_failing(self):
        ...
        self.site.latitude, self.site.longitude = 51.5, -0.12
        self.site.save()
        response = self.client.get(f'{reverse("plugins:netbox_atlas:world")}?overlay=no-such-thing')
        self.assertEqual(response.status_code, 200)
...
class TraceViewTest(ViewTestCase):
    def test_an_unknown_termination_is_a_404_not_a_500(self):
        response = self.client.get(reverse('plugins:netbox_atlas:trace', args=['dcim.interface', 10_000_000]))
    ...
    def test_it_refuses_a_user_who_cannot_read_cables(self):
        ...
        response = self.client.get(reverse('plugins:netbox_atlas:trace', args=['dcim.interface', 1]))
...
class BulkActionTest(ViewTestCase):
    ...
    def test_no_list_offers_an_unrouted_action(self):
        for url in (
            reverse('plugins:netbox_atlas:floor_list'),
            reverse('plugins:netbox_atlas:rackplacement_list'),
        ):
    ...
    def test_the_bulk_routes_resolve(self):
        for name in (
            ...
            with self.subTest(name=name):
                self.assertTrue(reverse(f'plugins:netbox_atlas:{name}'))

    def test_bulk_delete_removes_the_floors(self):
        response = self.client.post(
            reverse('plugins:netbox_atlas:floor_bulk_delete'),
    ...
    def test_deleting_a_floor_lands_somewhere_that_exists(self):
        ...
        response = self.client.post(
            reverse('plugins:netbox_atlas:floor_delete', args=[self.floor.pk]),
            {'confirm': True},
            follow=True,
        )
        self.assertEqual(response.status_code, 200)
...
class WorldColouringTest(AtlasTestCase):
    """
...
class WorldCircuitTest(AtlasTestCase):
    """
