...
from netbox_atlas.tests.test_views import grant
from netbox_atlas.tracing import trace_from


class TraceVisibilityTest(AtlasTestCase):
    def setUp(self):
    ...
    def test_the_view_answers_404_for_a_hidden_port(self):
        ...
        response = self.client.get(
            reverse('plugins:netbox_atlas:trace', args=['dcim.interface', self.b.interfaces.first().pk])
        )
        self.assertEqual(response.status_code, 404)


class RackCablingVisibilityTest(AtlasTestCase):
...
class ApiLayoutVisibilityTest(TestCase):
    ...
    def test_the_layout_holds_only_the_racks_the_reader_may_see(self):
        response = self.client.get(reverse('plugins-api:netbox_atlas-api:floor-layout', args=[self.floor.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual([rack['name'] for rack in response.json()['racks']], ['MINE'])


class WorldActionTest(AtlasTestCase):
    def test_a_site_with_no_floor_says_it_opens_the_site(self):
        ...
        self.assertEqual(node.url, reverse('dcim:site_atlas', args=[self.site.pk]))
