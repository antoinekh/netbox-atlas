...
from netbox_atlas.cabling import RESTRICTED_LABEL
from netbox_atlas.elevation import build_elevation
from netbox_atlas.scene import (
    EXIT_RISE_MM,
    LEFT,
    PLINTH_MM,
    RIGHT,
...
from netbox_atlas.tests.base import AtlasTestCase, cable, make_device, make_rack
...
class DeviceBoxTest(AtlasTestCase):
    def setUp(self):
...
class CableRouteTest(AtlasTestCase):
    def setUp(self):
...
class LeavingTheRackTest(AtlasTestCase):
    def setUp(self):
...
class RackViewTest(ViewTestCase):
    ...
    def test_the_tab_renders_with_its_scene(self):
        ...
        self.assertContains(response, 'id="atlas-rack3d-config"')
    ...
    def test_the_column_beside_it_is_rendered_up_front(self):
        response = self._get()
        self.assertContains(response, f'data-atlas-alloc="{self.device.pk}"')
        self.assertContains(response, 'data-atlas-trace-card')
