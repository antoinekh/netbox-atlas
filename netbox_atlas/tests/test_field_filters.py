...
from netbox_atlas.palette import NO_DATA
...
def offering(*names):
    ...
    config['netbox_atlas']['filter_custom_fields'] = list(names)
...
class FieldFilterPageTest(FieldFilterTestCase):
    def test_the_floor_offers_the_field_like_tags(self):
        ...
        self.assertContains(response, 'data-atlas-pick="pci"')
    ...
    def test_the_rack_table_has_a_column_for_the_field(self):
        with offering('compliancy'):
            response = self.client.get(self.floor.get_absolute_url())
        self.assertContains(response, '<button type="button" class="atlas-sort">Compliancy</button>')
    ...
    def test_a_field_with_no_value_in_use_offers_no_button(self):
        ...
        with offering('compliancy'):
            response = self.client.get(self.floor.get_absolute_url())
        self.assertNotContains(response, 'data-atlas-finder="field0"')
