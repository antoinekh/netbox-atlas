...
from netbox_atlas import filtersets, forms, tables
from netbox_atlas.cabling import kind_legend
from netbox_atlas.device_overlays import get_device_overlay, get_device_overlays
...
class FloorView(generic.ObjectView):
    ...
    template_name = 'netbox_atlas/floor.html'
...
class FloorLayoutView(generic.ObjectView):
    ...
    queryset = Floor.objects.all()
    template_name = 'netbox_atlas/floor_layout.html'
    tab = ViewTab(label='Edit layout', permission='netbox_atlas.change_rackplacement')
    ...
    def get_extra_context(self, request, instance):
        ...
        return {
            ...
            'can_add': request.user.has_perm('netbox_atlas.add_rackplacement'),
            'can_delete': request.user.has_perm('netbox_atlas.delete_rackplacement'),
            ...
            'grid_size': get_plugin_config('netbox_atlas', 'grid_size'),
            'layers': instance.layers.filter(enabled=True),
            'view': view,
            'x_ticks': metre_ticks(instance.width_cm, step),
            'y_ticks': metre_ticks(instance.depth_cm, step),
...
@register_model_view(Site, 'atlas', path='atlas')
class SiteAtlasView(generic.ObjectView):
    """
    The rooms in a site.
    ...
    template_name = 'netbox_atlas/site.html'
    tab = ViewTab(label='Atlas', permission='dcim.view_site', badge=lambda site: Floor.in_site(site).count())
    ...
    def get_extra_context(self, request, instance):
        ...
        return {
            ...
            'overlay': overlay,
            'overlays': get_overlays(),
            'unplaced': list(racks.filter(atlas_placement__isnull=True).order_by('name')),
        }
...
class RackAtlasView(generic.ObjectView):
    ...
    template_name = 'netbox_atlas/rack.html'
    tab = ViewTab(label='Atlas', permission='dcim.view_rack')
...
class WorldView(generic.ObjectListView):
    ...
    queryset = Site.objects.all()
    template_name = 'netbox_atlas/world.html'
    ...
    def get(self, request):
        ...
        overlay = get_site_overlay(request.GET.get('overlay')) or get_site_overlay(
            get_plugin_config('netbox_atlas', 'default_site_overlay')
        )
        world = build_world(
            self.queryset,
            overlay=overlay,
        ...
        # it has several, and the site's Atlas tab where it has none, which lists its racks and
        ...
        for node in world.nodes:
            ...
            if len(floors) == 1:
                ...
                node.url, node.action = reverse('dcim:site_atlas', args=[node.object_id]), 'Open the rooms'
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                # the page as markup.
                'map_config': {
                    'tileUrl': get_plugin_config('netbox_atlas', 'map_tile_url'),
                    'attribution': get_plugin_config('netbox_atlas', 'map_attribution'),
                    'mapCss': get_plugin_config('netbox_atlas', 'map_css'),
                    'mapJs': get_plugin_config('netbox_atlas', 'map_js'),
                    'glyphs': get_plugin_config('netbox_atlas', 'map_glyphs'),
