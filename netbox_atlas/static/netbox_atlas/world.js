...
(function () {
  ...
  const message = document.querySelector('[data-atlas-map-message]');
  const config = JSON.parse(document.getElementById('atlas-map-config').textContent);
  ...
  const status = document.querySelector('[data-atlas-world-status]');
  let selection = null;
  ...
  function inFocus(group, id) {
    return !window.atlasFinder || window.atlasFinder.inFocus(group, id);
  }
  ...
  document.addEventListener('click', function (event) {
    if (!event.target.closest('.atlas-map, .atlas-legend, [data-atlas-finder]')) {
      selection = null;
      apply();
    }
  });
  ...
  document.addEventListener('atlas:finder', function (event) {
  ...
  const legends = document.querySelector('[data-atlas-legends]');
  const legendsToggle = document.querySelector('[data-atlas-legends-toggle]');
  ...
  function hoverCard(node) {
    const rows = (node.facts || [])
      ...
      .join('');
    const state = node.status ? `<span class="atlas-card__status">${esc(node.status)}</span>` : '';
    const links = node.links
      ? `<div class="atlas-card__links">${node.links} circuit${node.links === 1 ? '' : 's'}</div>`
      : '';
    return (
      `<div class="atlas-card__head"><strong>${esc(node.label)}</strong>${state}</div>` +
      (rows ? `<table class="atlas-card__facts">${rows}</table>` : '') +
      links +
      `<div class="atlas-card__action">Click to ${esc(node.action.toLowerCase())}</div>`
    );
  }
  ...
  function bindMapEvents() {
    const popup = new maplibregl.Popup({ closeButton: false, className: 'atlas-card', offset: 12 });
    ...
    // Clicking a site goes into it. This map is the outermost level of an atlas, so moving
  ...
  async function buildMap() {
    ...
    try {
      ...
      console.warn('netbox-atlas:', error.message);
    }
    ...
    map.on('error', function (event) {
      console.warn('netbox-atlas map:', (event && event.error && event.error.message) || event);
    });
  ...
  buildMap().catch(function (error) {
    ...
    if (!error.forReader) console.warn('netbox-atlas could not draw the map:', error);
  });
})();
