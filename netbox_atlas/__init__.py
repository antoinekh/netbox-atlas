...
class AtlasConfig(PluginConfig):
    name = 'netbox_atlas'
    verbose_name = 'NetBox Atlas'
    ...
    default_settings = {  # noqa: RUF012
        ...
        # Where the 3D rack view loads Three.js from: a copy of the `three` npm package, laid out
        # as published, so `build/three.module.js` and `examples/jsm/` sit under it. Pinned,
        # because the addons under `examples/jsm/` must come from the same release as the core.
        # Empty turns the 3D tab's drawing off; the rest of the tab still works.
    ...
    def ready(self) -> None:
        ...
        selection = get_plugin_config('netbox_atlas', 'enable_builtin_overlays')
        if selection:
            register_builtin_overlays(list(selection) if isinstance(selection, (list, tuple, set)) else None)

        device_selection = get_plugin_config('netbox_atlas', 'enable_builtin_device_overlays')
        if device_selection:
            register_builtin_device_overlays(
                list(device_selection) if isinstance(device_selection, (list, tuple, set)) else None
            )

        site_selection = get_plugin_config('netbox_atlas', 'enable_builtin_site_overlays')
        if site_selection:
