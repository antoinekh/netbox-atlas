...
from netbox_atlas.overlays import NO_DATA_COLOUR as NO_DATA
from netbox_atlas.overlays import NO_DATA_LABEL, LegendEntry, Stat
from netbox_atlas.palette import NODE, distinct_colours
from netbox_atlas.site_overlays import get_site_overlay
...
class WorldNode:
    ...
    # its Atlas tab, or its one room where it has one; a provider network has no page.
    action: str = 'Trace'
...
def build_world(sites_queryset=None, overlay=None, circuits=None) -> WorldMap:
    ...
    for site in sites:
        ...
        nodes[f'site:{site.pk}'] = WorldNode(
            ...
            # The site's Atlas tab, which lists its racks and offers to add a floor. The view
            # replaces it with the room itself where the site has exactly one.
            url=reverse('dcim:site_atlas', args=[site.pk]),
