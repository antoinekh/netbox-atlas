...
from netbox_atlas.palette import COOLING, NO_DATA, UTILISATION, utilisation_colour
...
logger = logging.getLogger('netbox.plugins.netbox_atlas.overlays')
...
def floor_power_utilisation(racks: Sequence) -> dict[int, tuple[float, float]]:
    ...
    """
    return _memoised_on_racks(list(racks), '_atlas_power', _floor_power_utilisation)
...
def floor_space_utilisation(racks: Sequence) -> dict[int, tuple[float, float]]:
    ...
    """
    return _memoised_on_racks(list(racks), '_atlas_space', _floor_space_utilisation)
